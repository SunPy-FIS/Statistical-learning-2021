...
Football is one of the most important sports in all the world. Events like the World Championship are very popular and have a huge resonance on the media. We are now approaching the European Championship, and now it is simple to feel the importance of this event. Looking for an interesting theme for our project we considered the last decades rising of the value of top tier players. We are interested in understanding if the value can be related to the ability of a player. By this mean, we will analyze a dataset looking for this relationship. With the best model found we will look the most overrated and underrated players indicated by the model. The practical side of this analysis spaces from a basic usage in the rising field of e-sports to real field market considerations. Another application of our data is to use them to create three statistical models which result is more probable between win, draw and lose, based on the last Serie A tournament.   

The whole project is principally based on a _Kaggle_ dataset that contains the statistics about football players in the famous videogame FIFA (<https://www.kaggle.com/stefanoleone992/fifa-21-complete-player-dataset>). The choice of the dataset was determined by its cheapness and by the presence of many statistics useful for the videogame but difficult to get in the real world. It is difficult to assign a value from 0 to 100 regarding different abilities like defending, ball control or crossing to all the players in the world. FIFA game is bound in defining this values for the game. We take them like fictitious and simulated and we are interested in their possible usefulness in the real world. The only value strictly related to reality is the value of the players, which is taken from sites like <https://www.transfermarkt.it/uefa-champions-league/marktwerte/pokalwettbewerb/CL> at the release of the game. The choice of modeling the value based on the other "simulated" statistics is so a way to see if, in general, we can take them as valid for other analysis. The main project is going to focus on modeling the value, and at the end the two applications will be shown. The dataset used for the match result model was taken from <https://www.football-data.co.uk/italym.php>. In both the cases we used only data regarding the 2020/2021 championships. The match result model uses also some data taken from <http://clubelo.com/>.
...
In this application we are interested in creating a model that is able to predict the general result of a football match by the side of the home team: win, draw or lose. This is done by using three models that predicts the probability of winning, drawing or loosing. The higest probability is used to predict the result of that match. We are going to use a very simple model and see how it performs on the same data it was trained on. At the end we are going to compare the probability we are estimating with the intrinsic one given by the real quotes used by bookmakers. 
\
&nbsp;
\
&nbsp;
For this application a new dataset with match results was used and some new data, the elo ratings, were taken form a different site (see Introduction). As the site reports: "a club's elo rating is an estimation of its strength based on past results allowing predictions for the future". Elo ratings are a way for weighting our static teams statistics during the season, based on previous results. A small (inactive) preprocessing is reported, in which the result is encoded, given the goals on both parts and the elo values are calculated and correctly assigned to all the teams during all the season. Two new tables are then saved and loaded for the starting of the analysis.
...
\
&nbsp;
For the esteem of the models three new variables are created as vectors taking value 1 when the home team wins/draws/loses, and 0 otherwise. Only the 16 most important players of every team are selected by ommitting the team position _"SUB"_. For taking into account the strength of a team, the ***overall*** statistic is summed up for all the players in that team. This value is than rescaled for all the matches of the season by a weight calculated using the elo. This weight is equal to the percentage of change of elo rating with respect to beginning. The ***_overall_strength*** of each team thus calculated is then put in the right position into the matrix of the matches. For accounting the home advantage, a factor of 4.5 is added to the ***home_overrall_strength***.
...
To calculate the models, the difference between the ***overall_strengths*** of the two teams is calculated. This is used in in the models to fit respectively the wins, draws and losses. For their 0-1 encoding, the models used were generalized linear models of binomial family with logit link function. The summary of the models are reported. The only model with non significative parameters is that of the draws. In the other cases, the bias term is not significative but will not be removed. The interesting thing of this models is that the weigths calculated in the win and lose models are with very similar value and opposite sign. This means that the relationship home versus away is understood by the models.
...
The probabilities calculated are used for seeing how able is the model to repredict the same datas. The accuracy obtained is over 50% and is better than predicting always the same result. In the final plot the implicit probabilities coming from the quotes of home winning, calculated as the inverse of the quote, are compared with our home winning probability. The relationship between the two is good and means that our model provides probabilities similar to the ones calculated with more complex models by the bookmakers.
...
pred_win = predict(mod_win, type="response")
plot(pred_win, 1/quote)
abline(0,1, col="red")
```

## Conclusions

The realization of this project has been an interesting journey into the statistical analysis. From it we have learned how important is to be able to analyze the variables at the start in order to get a good model. In particular, we saw that sometimes transforming and grouping can really help in getting a simpler and more effective model. It has been good to see that sometimes a general sense model is not that bad and that by model selection and regularization one can obtain different models but with similar explicative power. Practical applications finished the job, giving us the satisfaction of using the model. 
\
&nbsp;

From the start, the modeling of the ***value*** variable was not simple. The problem was its distribution, but the most difficult part in the process was to obtain not too much distorted residuals. We know that the model get better by including the ***wage*** variable, but we were interested in the other statistics. The final model selected seems to get the most of the residuals close to be normally distributed with some exceptions. Our final idea is that the FIFA videogame dataset can be used in real applications with good results. As we saw in the applications, some tasks are more difficult than others, and so more useful variables could be required. We think the over/under-rated application has centered the point, while the match result prediction has to be improved to be really used. Many others applications can be found using this data, and more in general we know that sport data analytics is getting more importance in the last years. This brief project is in that trend, providing us a first experience in the field. 

## References



--------> sistema ultimo grafico

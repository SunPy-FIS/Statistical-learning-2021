...
selected_df <- df[df$league_rank & !random_idxs <= 1,-c(1,6,7,8,9,13,14,17,18,20,55,56,57,58,59)]
...
random_idxs <- runif(500,min=1,max=length(selected_df[,1]))
...
points(58,reg.summary$adjr2[58], col="red",cex=2,pch=20)

# Mallow's Cp with its smallest value
plot(reg.summary$cp,xlab="Number of Variables",ylab="Cp",type='l')
which.min(reg.summary$cp)
points(65,reg.summary$cp[65],col="red",cex=2,pch=20)

# BIC with its smallest value
plot(reg.summary$bic,xlab="Number of Variables",ylab="BIC",type='l')
which.min(reg.summary$bic)
points(23,reg.summary$bic[23],col="red",cex=2,pch=20)
...
points(47,back.summary$adjr2[47], col="red",cex=2,pch=20)

# Mallow's Cp with its smallest value
plot(back.summary$cp,xlab="Number of Variables",ylab="Cp",type='l')
which.min(back.summary$cp)
points(32,back.summary$cp[32],col="red",cex=2,pch=20)

# BIC with its smallest value
plot(back.summary$bic,xlab="Number of Variables",ylab="BIC",type='l')
which.min(back.summary$bic)
points(25,back.summary$bic[25],col="red",cex=2,pch=20)

par(mfrow=c(1,1))

variables <- coef(regfit.fwd,30)
#variables <- coef(regfit.back,29)
